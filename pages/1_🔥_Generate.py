...
st.set_page_config(page_title="Generate", page_icon="https://github.com/AnasMations/StudySync/blob/36ae1cad78544b8a07239d1cefa141a59f6305c8/img/icon.png?raw=true")
...
def GENERATE():
    ...
    input_options = ['Choose Input Type', 'Presentation slides', '.PDF', 'Video transcript', 'Topic / Question']
    input_selection = st.selectbox('Input Study Material:', input_options)
    input_type = input_options.index(input_selection)-1

    # Button selection for output_type
    output_options = ['Questions and Answers', 'Multiple choice questions', 'Summarization', 'Bullet points']
    output_selection = st.radio('What should I generate?', output_options)
    output_type = output_options.index(output_selection)

    # Slider for input_tokens
    input_tokens = st.slider("Select Complexity:", min_value=50, max_value=1000, value=250, step=50)
    ...
    if input_type == 0: #presentation         
        ...
        if uploaded_file is not None:
            ...
            if st.button("Generate"):
                with st.spinner(text="Generating Summary..."):
                    st.write(GenerateSummary(input_type, output_type, content, tokens=input_tokens))
        ...
        if url:
            if st.button("Generate"):
                with st.spinner(text="Generating Summary..."):
                    st.write(GenerateSummary(input_type, output_type, url, tokens=input_tokens))
        ...
        if user_input:
            if st.button("Generate"):
                with st.spinner(text="Generating Summary..."):
                    st.subheader(output_options[output_type])
                    st.write(GenerateSummary(input_type, output_type, user_input, tokens=input_tokens))
